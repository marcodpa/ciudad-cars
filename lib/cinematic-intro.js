...
export function mountCinematicIntro(root) {
  ...
  function syncLoading() {
    frames?.setActive(
      !disposed && !reduced && inView && !document.hidden,
    );
    requestDraw();
  }
  ...
  function makeMotion() {
    ...
    mediaContext.add(
      ...
      (match) => {
        ...
        reduced = match.conditions.reduce;
  ...
  for (const event of ['wheel', 'touchstart', 'pointerdown']) {
    listen(window, event, stopScroll, { passive: true });
  }
  ...
  root.querySelectorAll('a[href^="#"]').forEach((link) =>
    listen(link, 'click', (e) => {
      ...
      scrollTween = gsap.to(window, {
        ...
        onComplete: () => {
          scrollTween = null;
          if (link.matches('.cc-intro-skip, [data-skip-tour]')) {
